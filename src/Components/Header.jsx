...
const Header = () => {
    ...
    const [user, setUser] = useState(null); // Начальное значение null для пользователя
    const [isMenuOpen, setIsMenuOpen] = useState(false); // Состояние для открытия/закрытия меню
    const id = localStorage.getItem("userId");

    const [isMouse, setIsMouse] = useState(false)
    const [modBasket, setModBas] = useState([])
    ...
    const removeFromBasket = (itemId) => {
        axios.delete(`http://localhost:8081/Basket/${itemId}`)
            .then(() => {
                ...
                setModBas(prevBasket => prevBasket.filter(item => item.id !== itemId));
            })
            .catch(error => {
    ...
    useEffect(() => {
        if (searchQuery.length > 0) {
            ...
            document.getElementById('inputElement').style.borderBottomRightRadius = '0px';
            document.getElementById('inputElement').style.borderBottomLeftRadius = '0px';
        } else {
            document.body.style.overflow = 'auto';
            document.getElementById('inputElement').style.borderBottomRightRadius = '8px';
            document.getElementById('inputElement').style.borderBottomLeftRadius = '8px';
        }
    ...
    return (
        <div className="max-w-[1200px] mx-auto my-[26px]">
            <div className="flex items-center justify-between gap-[30px]">
                {/* Логотип */}
                <svg data-v-4430e31c="" onClick={() => navigate("/")} width="240" viewBox="0 0 215 32" fill="none" xmlns="http://www.w3.org/2000/svg" alt="Uzum" className="w-[240px] cursor-pointer">
  ...
  <path d="M52.1966 16.4854C52.1966 18.5564 51.0235 19.5164 49.2946 19.5164C47.5657 19.5164 46.4062 18.577 46.4062 16.4854V9.64166H42.6466V16.6157C42.6466 21.1622 46.4337 22.9383 49.3083 22.9383C52.1829 22.9383 55.9631 21.1554 55.9631 16.6157V9.64166H52.2035L52.1966 16.4854Z" fill="#7000FF"></path>
                ...
                {/* Кнопка Каталог */}
                <div className="flex items-center gap-[5px] bg-[#EAEAF9] !px-[20px] !py-[6px] rounded-[4px] cursor-pointer">
                    <div className="">
                        <div className="w-[14px] mx-auto h-[2px] bg-[#7f4dff] mb-[0.5px] rounded-t-[12px]"></div>
                        <div className="w-[16px] mx-auto h-[2px] bg-[#7f4dff] mb-[0.5px] rounded-t-[12px]"></div>
                        <div className="w-[18px] h-[12px] border-[2px] border-[#7f4dff] "></div>
                    </div>
                    <button className="text-[14px] text-[#3333CC] hidden md:block" >
                        Каталог
                    </button>
                </div>
                ...
                <div className="relative hidden md:block">
                    <form>
                        <input
                            ...
                            value={searchQuery}
                            className="text-[13px] px-[15px] py-[6px] border w-[500px] rounded-[8px]"
                            id="inputElement"
                        />
                        <img
                            src={searchQuery.length > 0 ? "/icons/cancel.png" : "/icons/search.png"}
                            alt="search"
                            className="absolute top-2 right-4 w-[15px] cursor-pointer"
                            onClick={() => clearInpit()}
                        />
                    </form>
                    {
                        searchQuery.length > 0 ? <Search searchProd={searchProd} searchQuery={searchQuery} clearInpit={clearInpit} /> : ""
                    }
                </div>
                ...
                <div
                    ...
                    } absolute top-[80px] right-0 w-[250px] bg-white shadow-lg p-[20px] rounded-[8px] md:hidden`}
                >
                    <div className="flex flex-col items-center">
                        ...
                        <div className="flex items-center gap-2 mb-[10px]">
                            ...
                            <span className="bg-[#7000FF] px-[5px] rounded-[4px] text-white">
                ...
                <div className="hidden md:flex items-center gap-[21px]">
                    ...
                    <div className="flex items-center gap-2">
                        <p
                            className="cursor-pointer hover:bg-gray-200 px-[10px] py-[3px] rounded-[4px]"
                            onClick={() => navigate("/Favorite")}
                        >
                            Избранное
                        </p>
                    ...
                    <div className="flex items-center relative gap-2">
                        <div className="" 
                                    onMouseEnter={() => setIsMouse(true)}  // при наведении
                                    onMouseLeave={() => setIsMouse(false)} // при уходе курсора
                            >
                            <span
                                className="cursor-pointer hover:bg-gray-200 px-[10px] py-[5px] rounded-[4px]"
                                onClick={() => navigate("/Basket")}>
                                Корзина
                            </span>
                            
                            {
                                isMouse ? <div className="bg-[#fffffffd] rounded-b-[20px] rounded-tl-[20px] p-[10px] pb-0 right-0 absolute z-50">
                                    {/* modBasket */}
                                    <div className="">
                                        {
                                            modBasket.map((data, i) => {
                                                return (
                                                    <BasketModal data={data} key={i} removeFromBasket={removeFromBasket} />
                                                )
                                            })
                                        }
                                    </div>
